...
`VirtualCOM-GUI.exe`，布局按功能规格第 3 节：

- 顶部是标题与「实现方式：用户态（无驱动）」按钮，点它看实测的兼容性边界
- 每对串口一张卡片，两端对称展示端口号、状态、占用程序和 TX 字节数，中间是 `⇄` 配对标识
- 每张卡片带 `详情`、`修改编号`、`禁用/启用`、`重启`、`删除` 五个操作，删除会先列出将要删掉的两个编号让你确认
- 底部显示串口对与虚拟端口数量，以及全局 `诊断` 和 `刷新`
- 没有串口对时显示空状态与创建入口

命令行参数 `--pair COM10:COM11` 可重复，用于带预设串口对启动。

与规格的两处差异，都是因为本版没有驱动：顶部的「驱动状态」改成了「实现方式」，因为本方案不装驱动，显示驱动状态是假信息；「更多 ⋯」展开成一排文字按钮，规格本就要求删除用文字标识而非图标，摊开比藏进菜单更清楚。
...
| 第 3 节 主界面（卡片、对称展示、空状态、底部计数与诊断刷新） | 完成，见上文两处差异说明 |
