...
func TestBuiltVersionResource(t *testing.T) {
	...
	if got := exeFileVersion(t, exe); got != wincore.Version {
		t.Fatalf("exe 版本资源 = %q, 版本常量 = %q;改版本号后需要重新生成 .syso(goversioninfo -64 -o rsrc_windows_amd64.syso versioninfo.json)", got, wincore.Version)
	}
}

// TestVersionInfoJSONMatchesConstant 防止改了版本常量却漏改某个 versioninfo.json。
func TestVersionInfoJSONMatchesConstant(t *testing.T) {
	for _, path := range []string{"versioninfo.json", filepath.Join("..", "versioninfo.json")} {
		...
		if vi.StringFileInfo.FileVersion != wincore.Version || vi.StringFileInfo.ProductVersion != wincore.Version {
			t.Errorf("%s 声明 %q/%q, 版本常量 = %q", path, vi.StringFileInfo.FileVersion, vi.StringFileInfo.ProductVersion, wincore.Version)
		}
	}
}
